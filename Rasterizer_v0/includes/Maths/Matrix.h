...
public:
	...
	static Matrix<T, 3, 3> CreateRotationMatrix(const Vector3<T>& p_rotation)
    {
		return  Matrix<T, 3, 3>::CreateXRotationMatrix(Vec3<T>::ToRad(p_rotation.X())) * 
				Matrix<T, 3, 3>::CreateYRotationMatrix(Vec3<T>::ToRad(p_rotation.Y())) *
				Matrix<T, 3, 3>::CreateZRotationMatrix(Vec3<T>::ToRad(p_rotation.Z()));
    }

	static Matrix<T, 4, 4> CreateTransformMatrix(const Vector3<T>& p_rotation, const  Vector3<T>& p_position, const  Vector3<T>& p_scale)
    {
    ...
    Matrix<T, R, C> Multiply(const std::vector<T> &p_other) const
    {
        Matrix<T, R, C> multiply;
        if (p_other.size() != C)
    ...
    void Multiply(const std::vector<T> &p_other)
    {
        if (p_other.size() != C)
            std::cout << "Can't be multiply" << std::endl;
        else
    ...
    Matrix<T, R, C> Transpose() const
    {
		...
		// REFACTOR Integrate in basic Matrices multiply
    T VectorsMul(const std::vector<T> &p_v1, const std::vector<T> &p_v2) const
    {
        T VectorsMul = 0;
        if (p_v1.size() != p_v2.size())
            std::cout << "Can't be multiply" << std::endl;
        else
        {
            auto it1 = p_v1.begin();
...
//			}
	...
	const T* GetRow(unsigned int p_row) const
	{
		if (p_row < R)
		{
			return this->m_matrix[p_row];
		}
		std::cout << "Out of Matrix" << std::endl;
		return nullptr;

	}
	T* GetRow(unsigned int p_row)
	{
		if (p_row < R)
		{
			return this->m_matrix[p_row];
	...
	const T* GetColumn(unsigned int p_col) const
	{
		T col[C];
		if (p_col < C)
		{
			for (int i = 0; i < R; ++i)
				col[i] = this->m_matrix[i][p_col];
			return col;
		}
		std::cout << "Out of Matrix" << std::endl;
		return nullptr;
	...
	T* GetColumn(unsigned int p_col)
	{
		T col[C];
		if (p_col < C)
		{
    ...
    Matrix<T, R, C> operator*(const std::vector<T> &p_other) const { return Multiply(p_other); }
    Matrix<T, R, C> operator*=(const std::vector<T> &p_other) { return Multiply(p_other); }
